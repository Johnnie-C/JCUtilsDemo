...
- (void)loadView {
    ...
    if(UI_USER_INTERFACE_IDIOM() != UIUserInterfaceIdiomPad){ //iphone
        ...
        passcodeTextField = [[NumericKeypadTextField alloc] init];
    }
    ...
    passcodeTextField.inputAssistantItem.leadingBarButtonGroups = @[];
    passcodeTextField.inputAssistantItem.trailingBarButtonGroups = @[];
    ...
    if(_action == PasscodeActionEnter){
        btnForgorPasscode = [[UIButton alloc] init];
        btnForgorPasscode.translatesAutoresizingMaskIntoConstraints = NO;
        [btnForgorPasscode setTitle:@"Forgot Passcode" forState:UIControlStateNormal];;
        [btnForgorPasscode setTitleColor:[UIColor darkTextColor] forState:UIControlStateNormal];
        [btnForgorPasscode setTitleColor:[UIColor darkGrayColor] forState:UIControlStateHighlighted];
        btnForgorPasscode.titleLabel.font = [UIFont systemFontOfSize:14];
        [btnForgorPasscode addTarget:self
                              action:@selector(onForgotPasscodeClicked)
                    forControlEvents:UIControlEventTouchUpInside];
        [contentView addSubview:btnForgorPasscode];
        [btnForgorPasscode addHeightConstraint:50];
        [btnForgorPasscode alignParentBottomFillWidth];
    }
...
- (void)viewDidLoad {
    ...
    if ([_delegate respondsToSelector:@selector(PAPasscodeViewControllerDidCancel:)]) {
        if (_simple) {
            ...
            if(_rightCancelColor){
                [item setTitleTextAttributes:@{NSForegroundColorAttributeName:_rightCancelColor}
        ...
        case PasscodeActionSet:
            if (phase == 0) {
                ...
                if ([text isEqualToString:_passcode]) {
                    ...
                    if ([_delegate respondsToSelector:@selector(PAPasscodeViewControllerChangePasscodeError:)]) {
                        [_delegate PAPasscodeViewControllerChangePasscodeError:@"Passcodes did not match. Try again."];
                    }
                }
            }
            break;
...
- (BOOL)isPasscodeValid:(NSString *)passcode{
    if(!_delegate){
        [NSException raise:@"PAPasscode" format:@"PAPasscodeViewController must have a delegate"];
    }
    
    return [_delegate isPasscodeValid:passcode];
}

- (void)onForgotPasscodeClicked{
    if(_delegate){
        [_delegate PAPasscodeViewControllerOnForgotPasscodeClicked:self];
    }
}
